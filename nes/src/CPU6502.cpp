...
{
	uint8_t vector_lsb = *(this->bus.read(0xFFFC)); // Setting program counter to address
	...
	for(int i = 0; i <= 16; i++)
		this->bus.write(0x4000 + i, 0);
	
	for(int i = 0; i <= 3; i++)
		this->bus.write(0x4010 + i, 0);
}
...
{ 
	...
	uint8_t instr_opcode = *(this->bus.read(this->pc)); 
	...
	if(operand_bytes == 1)
	{
		this->instr_operand = *(this->bus.read(this->pc));
		this->pc++;
	}
	else if(operand_bytes == 2)
	{
		this->instr_operand = *(this->bus.read(this->pc));
		this->pc++;
		this->instr_operand += *(this->bus.read(this->pc)) << 8;
		this->pc++;
	}
	...
	/* If instruction mode is immediate, then variable with value
	   of the address is allocated on the heap, after being used
	   it's should be deallocated */

	if(cur_instruction.adr_mod == &CPU6502::mod_imd)
...
void CPU6502::reset()
{
	...
	{
		uint8_t vector_lsb = *(this->bus.read(0xFFFC));
		uint8_t vector_msb = *(this->bus.read(0xFFFD));
...
void CPU6502::irq()
{
	if(this->extract_flag(InterruptDisable) == false)
	{
		uint8_t vector_lsb = *(this->bus.read(0xFFFE));
		uint8_t vector_msb = *(this->bus.read(0xFFFF));
...
void CPU6502::nmi()
{
	...
	uint8_t vector_msb = *(this->bus.read(0xFFFB));
...
void CPU6502::ADC()
{
	uint8_t memory_data = *this->data_address;
...
void CPU6502::ASL() 
{
	this->set_flag(Carry, *this->data_address >= 128);

	*this->data_address <<= 1;

	this->set_flag(Zero, *this->data_address == 0);
	this->set_flag(Negative, *this->data_address >= 128);	
}
...
void CPU6502::LSR()
{
	this->set_flag(Carry, *this->data_address % 2);
...
void CPU6502::STA()
{
	*this->data_address = this->acc;
}

void CPU6502::STX()
{
	*this->data_address = this->x;
}

void CPU6502::STY()
{
	*this->data_address = this->y;
}
